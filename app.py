...
def calculate_options():
    servers = pd.read_csv(os.path.join(BASE_DIR, 'servers.csv')).fillna(0)
    server_modules = pd.read_csv(os.path.join(BASE_DIR, 'server_modules.csv')).fillna(0)
    accessories = pd.read_csv(os.path.join(BASE_DIR, 'accessories.csv')).fillna(0)
    data = request.json
...
def generate_reports():
    controllers = pd.read_csv(os.path.join(BASE_DIR, 'controllers.csv')).fillna(0)
    servers = pd.read_csv(os.path.join(BASE_DIR, 'servers.csv')).fillna(0)
    server_modules = pd.read_csv(os.path.join(BASE_DIR, 'server_modules.csv')).fillna(0)
    accessories = pd.read_csv(os.path.join(BASE_DIR, 'accessories.csv')).fillna(0)
    for df in [controllers, servers, server_modules]: df['Name'] = df['Name'].str.strip()
    data = request.json
    panel_choices = data.get('panel_choices'); standard_panels = data.get('standard_panels')
...
if __name__ == '__main__':
    # This part is for local testing. Railway will use the Procfile.
    # The port needs to be read from an environment variable for Railway.
